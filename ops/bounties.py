...
RewardType = str  # one of: "shards", "diamonds", "gold", "scrap"
...
class BountyManager:
    def __init__(self) -> None:
        ...
        # Pool of possible bounties (mix of currencies and activities)
        self._pool: Dict[str, BountyTemplate] = {
            ...
            "daily_slots_wins_5":       BountyTemplate("daily_slots_wins_5", "Win Slots 5x",                "daily",  "counter_slots_wins",      5,      "gold",     5000),
            "daily_roulette_win_5":     BountyTemplate("daily_roulette_win_5", "Win Roulette 5x",            "daily",  "counter_roulette_wins",   5,      "shards",   3000),
            "daily_roulette_spins_30":  BountyTemplate("daily_roulette_spins_30", "Spin Roulette 30x",        "daily",  "counter_roulette_spins",  30,     "scrap",    800),
            "daily_dice_plays_200":     BountyTemplate("daily_dice_plays_200", "Roll Dice 200x",             "daily",  "counter_dice_plays",      200,    "shards",   1200),
            "daily_convert_2000_scrap": BountyTemplate("daily_convert_2000_scrap", "Convert 2,000 Scrap",      "daily",  "scrap",                   2000,   "shards",   2500),
            "daily_earn_25k_gold":      BountyTemplate("daily_earn_25k_gold", "Earn 25,000 gold",            "daily",  "gold",                    25_000, "diamonds", 3),
            "daily_open_5_crates":      BountyTemplate("daily_open_5_crates", "Open 5 Crates",               "daily",  "crates_total",            5,      "shards",   2000),

            # ----- Weekly -----
            "weekly_crates_20":          BountyTemplate("weekly_crates_20", "Open 20 Crates",               "weekly", "crates_total",            20,     "shards",   5000),
            "weekly_crates_60":          BountyTemplate("weekly_crates_60", "Open 60 Crates",               "weekly", "crates_total",            60,     "diamonds", 20),
            ...
            "weekly_roulette_spins_200": BountyTemplate("weekly_roulette_spins_200", "Spin Roulette 200x",      "weekly", "counter_roulette_spins",  200,    "gold",     150_000),
            "weekly_slots_spins_300":    BountyTemplate("weekly_slots_spins_300", "Spin Slots 300x",           "weekly", "counter_slots_spins",     300,    "scrap",    5000),
            ...
            "weekly_lifetime_gold_2m":   BountyTemplate("weekly_lifetime_gold_2m", "Earn 2,000,000 lifetime gold", "weekly", "lifetime_gold", 2_000_000, "diamonds", 60),
        }
    ...
    def _ensure_roll(self) -> None:
        ...
        if now >= self.daily_reset_at:
            ...
            self.daily_keys = self._roll("daily", 3)
            self.daily_reset_at = self._next_daily()
        if now >= self.weekly_reset_at:
            self.weekly_claimed.clear()
            self.weekly_keys = self._roll("weekly", 3)
            self.weekly_reset_at = self._next_weekly()
    ...
    def _progress(self, game, metric: str) -> float:
        # Accept counters or totals accessible on Game.
        # Special handling for some virtual metrics.
        if metric == "crates_total":
            try:
                co = getattr(game, "crates_opened", {})
                return float(sum(int(v) for v in co.values()))
            except Exception:
                return 0.0
        # For currencies and counters, return the current attribute value
    ...
    def list(self, game) -> List[dict]:
        ...
        for cat, keys, claimed in (
            ...
            for key in keys:
                ...
                if not t: 
                    continue
                cur = self._progress(game, t.metric)
                out.append({
                    "key": key,
                    "name": t.name,
                    "category": cat,
