...
class InventoryScreen(QWidget):
    def __init__(self, main_window):
        ...
        self.loadout.show_in_inventory.connect(self.inv.select_uid)
    ...
    def _equip_uid(self, uid: int):
        # Prefer replacing same-sides die if present; else equip into empty slot
        changed = False
        try:
            if hasattr(self.game, 'equip_replace_or_empty'):
                changed = self.game.equip_replace_or_empty(uid)
            else:
                changed = self.game.equip_first_empty(uid)
        except Exception:
            changed = self.game.equip_first_empty(uid)
        if changed:
            self.loadout.refresh()
