...
from core.dice_models import get_templates
from .ui_icon_util import dice_icon_with_stars
...
class CrateRevealDialog(QtWidgets.QDialog):
    def __init__(self, game, tier: str, inst, parent=None):
        ...
        img_path = None
        try:
            rp = t.resolve_icon_path() if t else None
            img_path = rp.as_posix() if rp else None
        except Exception:
            img_path = None
        label = f"d{t.sides}" if t else None
        qpm = dice_icon_with_stars(img_path, 96, stars=0, label_text=label)
        if not qpm.isNull():
            icon.setPixmap(qpm)
    ...
    def showEvent(self, e):
        super().showEvent(e)
        # Hide "Open Another" unless this is a scrap crate tier
        allowed = {"basic", "advanced", "rare", "legendary"}
        if str(self.tier).lower() not in allowed:
            self.btn_again.hide()
