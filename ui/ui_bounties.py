...
class BountiesDialog(QtWidgets.QDialog):
    ...
    def refresh(self):
        ...
        for b in self.game.list_bounties():
            # Build reward label by type
            rtype = str(b.get('reward_type','shards'))
            amt = int(b.get('reward', 0)) if rtype != 'gold' else int(b.get('reward', 0))
            suffix = {
                'shards': 'shards',
                'diamonds': 'diamonds',
                'gold': 'gold',
            ...
            reward_txt = f"{amt:,} {suffix}"
            text = f"{b['name']}  |  {int(min(b['current'], b['target']))}/{int(b['target'])}  |  Reward: {reward_txt}"
    ...
    def _on_click(self, it: QtWidgets.QListWidgetItem):
        ...
        if data and data['done'] and not data['claimed']:
            if self.game.claim_bounty(key):
                rtype = str(data.get('reward_type','shards'))
                amt = int(data.get('reward', 0)) if rtype != 'gold' else int(data.get('reward', 0))
                suffix = {
                    'shards': 'shards',
                    'diamonds': 'diamonds',
                    'gold': 'gold',
                ...
                QtWidgets.QToolTip.showText(self.mapToGlobal(self.rect().center()), f"Claimed +{amt:,} {suffix}!")
                self.refresh()
        else:
