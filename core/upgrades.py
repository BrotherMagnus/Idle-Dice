...
class UpgradeDef:
    ...
    # Milestone gating for building-specific upgrades
    # Example: appears after owning N of a base building
    ...
    # Scrap
    scrap_passive: float = 0.0           # scrap/sec
    salvage_yield_mult: float = 0.0      # +% scrap from salvage per level (0.10 = +10%)
    salvage_cost_discount: float = 0.0   # -% salvage cost per level (0.02 = -2%)
...
UPGRADES: List[UpgradeDef] = [
    ...
    # ---------- BUILDINGS (milestone upgrades) ----------
    UpgradeDef(
        key="kiosk_efficiency_25", category="buildings", name="Kiosk Efficiency I",
        base_cost=25000, cost_multiplier=1.2, max_level=1,
        building_gold_ps=5.0,
        milestone_key="b_kiosk", milestone_level=25,
        description="Bulk ordering and optimized layouts increase kiosk revenue."
    ),
    UpgradeDef(
        key="kiosk_efficiency_50", category="buildings", name="Kiosk Efficiency II",
        base_cost=120000, cost_multiplier=1.25, max_level=1,
        building_gold_ps=20.0,
        milestone_key="b_kiosk", milestone_level=50,
        description="Advanced kiosks with faster payouts and clever upsells."
    ),
    UpgradeDef(
        key="bar_mixology_25", category="buildings", name="Signature Mixology",
        base_cost=80000, cost_multiplier=1.25, max_level=1,
        building_gold_ps=15.0,
        milestone_key="b_bar", milestone_level=25,
        description="Unique cocktails boost bar margins and attract regulars."
    ),
    
    # ---------- SCRAP ----------
    UpgradeDef(
        key="scrap_idle_1", category="scrap", name="Workshop Tools",
        base_cost=1200, scrap_passive=0.2, cost_multiplier=1.15, max_level=50,
        description="Basic tools generate scrap slowly over time."
    ),
    UpgradeDef(
        key="scrap_idle_2", category="scrap", name="Auto Shredder",
        base_cost=8000, scrap_passive=1.0, cost_multiplier=1.18, max_level=40,
        reveal_after_key="scrap_idle_1", reveal_after_level=10,
        description="Automated shredders increase passive scrap output."
    ),
    UpgradeDef(
        key="scrap_yield_1", category="scrap", name="Better Salvage",
        base_cost=5000, salvage_yield_mult=0.10, max_level=20,
        description="Salvage yields +10% more scrap per level."
    ),
    UpgradeDef(
        key="scrap_discount_1", category="scrap", name="Bulk Contracts",
        base_cost=6000, salvage_cost_discount=0.02, max_level=25,
        description="Salvage costs -2% per level (caps at 95%)."
    ),
]
