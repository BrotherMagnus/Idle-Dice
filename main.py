...
class GameScreen(QWidget):
    """Play area: Dice / Slots / Roulette (when unlocked)."""
    def __init__(self, main_window: "MainWindow"):
        super().__init__(main_window)
        self.mw = main_window
    ...
    def refresh_all(self):
        self.gold_label.setText(f"Gold: {int(self.game.gold)} | Diamonds: {self.game.diamonds} | Shards: {int(self.game.shards)}")
        self.dice_label.setText(f"Dice: {self.game.dice_count}d{self.game.die_sides}")

        # Add tabs upon unlock
    ...
    def on_bet_clicked(self):
        ...
        self._timer.setInterval(max(30, int(150 * self.game.animation_speed)))
...
# ---------------- Main Window (navigation + top bar) ----------------
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.stack.addWidget(self.hub)             # index 1
        self.stack.addWidget(self.games)           # index 2
    ...
    def show_game(self, direct_tab: str | None = None):
        ...
        if direct_tab:
            self.game_play.select_tab(direct_tab)
    ...
    def open_building_upgrades(self):
        dlg = UpgradesDialog(self.game, self, category_filter="buildings", title="Building Upgrades")
        dlg.exec()
        self.games.refresh()
    ...
    # -------- Tick & save --------
    def tick(self):
        # unified passive tick (slots + roulette + buildings + shards)
        self.game.tick_passive()
        # ensure unlock flags flip as soon as thresholds are crossed
